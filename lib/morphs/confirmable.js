 ...
 * @author lykmapipo
 *
 * @description apply confirmable to a given sails model
 *
 ...
 * @public
 */
Confirmable.prototype.apply = function(model) {
    //mix in confirmable attributes
    this.mixin.attributes(model);

    //mix instance methods
    this.mixin.instanceMethods(model);
...
Confirmable.prototype.mixin = {
    ...
    instanceMethods: function(model) {
        //bind  generate confirmation token
        //as model instance method
        Helpers.mixAttributes(model, {
            generateConfrimationToken: exports._generateConfirmationToken
        });

        //bind send confirmation
        //as model instance method
        Helpers.mixAttributes(model, {
            sendConfirmation: exports._sendConfirmation
        });
    },

    staticMethods: function(model) {
        // body...
    }
};
 ...
 * @param {sendConfrimation~callback} done callback that handles the response.
 * @private
 */
Confirmable.prototype._sendConfirmation = function(next) {
 ...
 * @param {Object} confrimable a confirmable instance with `confirmationSentAt`
